...
func _check_physics_body(target: Node3D) -> void:
	if target is PhysicsBody3D:
		...
		Engine.get_version_info().minor < 4:
			if show_jitter_tips == null: # Default value is null when referencing custom Project Settings
		...
		elif not physics_interpolation_enabled and show_jitter_tips == null: # Default value is null when referencing custom Project Settings
